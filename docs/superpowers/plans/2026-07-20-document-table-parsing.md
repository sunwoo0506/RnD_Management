...
**⏸ 2026-07-21 기준 보류(Task 10~12):** HWPX·PDF(Task 1~9)는 완료·검증됐다. 실제로 확인해보니 최근 공고문·지침은 대부분 HWPX(신형식)로 배포되고 있어, 리스크가 가장 크고 검증되지 않은 HWP 바이너리 작업(Task 10~12)의 우선순위를 낮추고 잠시 보류하기로 했다. `scripts/inspect-hwp-table.mjs`(Task 10 Step 1)는 작성해뒀지만, 실제 표가 포함된 .hwp 샘플로 돌려 태그 번호를 확정하는 Step 2부터는 실행하지 않았다 — 다음에 HWP 지원이 필요해지면 이 스크립트에 실제 샘플 경로만 넣어 재개하면 된다. 그때까지 HWP 파일의 표는 기존 동작(구조 없이 문단 텍스트로만 추출)을 유지한다 — 회귀 없음, 그냥 개선이 아직 안 된 상태.
...
- ⏸ HWP 표 구조 보존 (병합 포함, 검증 우선) — Task 10~12, **보류** (최근 문서가 대부분 HWPX로 배포돼 우선순위 하향. 검증 스크립트만 작성해둠, 재개 시 실제 샘플로 Task 10부터 이어서 진행)
