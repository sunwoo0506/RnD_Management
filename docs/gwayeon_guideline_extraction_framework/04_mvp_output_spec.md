...
### 총액 한도에 연 한도가 함께 붙을 때 — `limit_value_per_year`

사업비 총액 한도를 정하면서 연 한도를 함께 두는 사업이 있다.

> 디딤돌 — 최대 1년 6개월 이내 / 정부지원연구개발비 **최대 2억원 이내 (최대 年 1억원 이내)**

이때 연 한도를 `ui_summary` 문장 안에만 적으면 **아무 데도 반영되지 않는다.** 실제로 디딤돌은
총액 2억만 실려 있어서, 사업기간이 1년인 과제에도 화면이 2억까지 쓸 수 있다고 안내했다.
1년짜리 과제의 실제 한도는 1억이다.

`limit_value` 옆에 `limit_value_per_year`(원 단위)를 함께 적는다. 화면은 사업기간이 걸친
연차 수(12개월당 1년차, 남는 달도 한 연차)를 곱해 **둘 중 작은 값**을 그 과제의 한도로 쓴다.

```json
{
  "limit_code": "DD_GOV_FUND_MAX_200M",
  "limit_type": "FIXED_AMOUNT",
  "limit_value": 200000000,
  "limit_unit": "KRW",
  "limit_value_per_year": 100000000,
  "basis_code": "gov_funded_cost",
  "ui_summary": "정부지원연구개발비는 최대 2억원 이내(연 1억원 이내), 지원기간은 최대 1년 6개월"
}
```
...
|---|---|---|
| 1년 | 1 | 1억 (연 한도가 먼저 걸림) |
| 1년 6개월 | 2 | 2억 (총액이 먼저 걸림) |

연 한도가 없는 사업은 이 필드를 넣지 않는다 — 넣지 않으면 총액 한도가 그대로 쓰인다.
