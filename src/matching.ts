...
const KEYWORDS: Record<string, Record<string, number>> = {
  prestartup: {
    ...
    '통합관리지침': 3, '사업화 자금': 3, '전담기관': 1, '창업기업': 1, '주관기관': 1,
  },
  'rnd-forprofit': {
    '영리기관': 4, '기업부담금': 3, '중소기업': 1, '중견기업': 1, '현물': 1,
  },
  'rnd-govt': {
    '정부출연기관': 5, '정부출연연구기관': 5, '출연연': 4, '학생인건비': 4, '학생연구자': 3, '기본사업': 2, '출연금': 2,
  },
};
...
export const classifyProgram = (text: string): MatchResult => {
  ...
  for (const [packId, keywords] of Object.entries(KEYWORDS)) {
    const merged = packId.startsWith('rnd') ? { ...RND_COMMON, ...keywords } : keywords;
  ...
  const merged = firstId.startsWith('rnd') ? { ...RND_COMMON, ...KEYWORDS[firstId] } : KEYWORDS[firstId];
