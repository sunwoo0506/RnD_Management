...
export const spendingMatrix = (pack: RulePack, project: Project, months: string[]): SpendingMatrix => {
  ...
  const leafCells = (categoryId: BudgetCategoryId, subItemId: string | undefined, budget: number, expenses: Expense[]): MonthCell[] => {
    const auto = splitEvenly(budget, allMonths.length);
    const actual = actualByMonth(expenses);
    return months.map((month) => {
      const index = indexOf.get(month);
      return {
        month,
        plan: overrideOf(categoryId, subItemId, month) ?? (index == null ? 0 : auto[index] ?? 0),
  ...
  const rows: MatrixRow[] = categorySpending(pack, project).map((row) => {
    ...
    const subRows: MatrixSubRow[] = row.subRows.map((sub) => ({
      ...sub,
      planEditable: !!sub.subItemId && !sub.orphan,
      cells: leafCells(row.categoryId, sub.subItemId, sub.budget,
    ...
    return {
      ...
      cells: divided ? addCells(subRows, months) : leafCells(row.categoryId, undefined, row.budget, expenses),
      subRows,
    };
  });
